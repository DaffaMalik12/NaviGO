...
const TabButton = ({ label, iconName, active, onClick }) => {
  ...
  const Icon = ({ iconName }) => {
    ...
    React.useEffect(() => {
      ...
      icons[iconName]().then(mod => {
        if (isMounted) setIcon(React.createElement(mod, { className: "h-4 w-4 mb-1" }));
      });
  ...
  return (
    <button 
      className={`flex-1 py-3 text-sm font-medium ${active ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
      onClick={onClick}
    >
